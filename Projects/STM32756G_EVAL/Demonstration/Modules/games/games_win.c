...
static int _ShowMessageBox(WM_HWIN hWin, const char* pTitle, const char* pText, int YesNo) {
  ...
  int r = 0;
  /* Create frame win */
  hFrame = FRAMEWIN_CreateEx(185, 180, 270, 120, hWin, WM_CF_SHOW, FRAMEWIN_CF_MOVEABLE, 0, pTitle, &_cbMessageBox);
  ...
  for (yPos = _BoardY0, y = 0; y < 8; y++) {
    for (xPos = _BoardX0, x = 0; x < 8; x++) {
      ...
      if (Cell || IsValidMove) {
        ...
        if (Cell) {
          Color = (Cell == 1) ? (0x00522000) : (0x00DCA939);
          LCD_SetColor(Color);
          #if (AA_FACTOR > 1)
...
static void Startup(WM_HWIN hWin, uint16_t xpos, uint16_t ypos)
{
  ...
  _hFrame = WINDOW_CreateEx(xpos, ypos, 640, 480, hWin, WM_CF_SHOW, 0, 0x500, &_cbReversiWin); 
  hItem = BUTTON_CreateEx(570, 0, 70, 70, _hFrame, WM_CF_SHOW, 0, ID_BUTTON_EXIT);
