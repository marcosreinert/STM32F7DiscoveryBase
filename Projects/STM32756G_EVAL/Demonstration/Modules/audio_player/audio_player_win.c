          ...
          if(((strstr(fn, ".wav")) || (strstr(fn, ".WAV"))))
          {
            ...
            if((update == 1) && (hMainWin != 0))
            {
              if(AUDIOPLAYER_GetFileInfo(tmp, &WavInfo) == 0)
              {
                ...
                LISTVIEW_SetItemTextColor(hItem, 1, WavList.ptr, LISTVIEW_CI_UNSEL, 0x00522000);              
              }
            }
            WavList.ptr++;
          }
...
/**
  ...
  * @retval None
  */
static void _cbEqualDialog_des(WM_MESSAGE * pMsg) {
  int     NCode;
  int     Id;
  WM_HWIN hItem;
  char tmp[] = "00:00";    
...
static void _cbMainDialog(WM_MESSAGE * pMsg) {
  ...
  case WM_INIT_DIALOG:
    ...
    if(WavList.ptr > 0)
    {
      while(ItemNbr < WavList.ptr)
      {
        if(AUDIOPLAYER_GetFileInfo((char *)WavList.file[ItemNbr].name, &WavInfo) == 0)
        {
          ...
          LISTVIEW_SetItemTextColor(hItem, 1, ItemNbr, LISTVIEW_CI_UNSEL, 0x00522000);              
    ...
    SLIDER_SetFocusColor(hItem, 0x00522000);
    SLIDER_SetBkColor (hItem, GUI_WHITE);
  ...
  case WM_NOTIFY_PARENT:
      ...
      {        
        if(AUDIOPLAYER_GetState() == AUDIOPLAYER_STOP)
        {
          ...
          else
          {
            ...
            hItem = CHOOSEFILE_Create(pMsg->hWin,  95, 115, 450, 250, apDrives, GUI_COUNTOF(apDrives), 0, "Open an audio file", 0, &FileInfo); 
            WM_MakeModal(hItem);
    ...
    case ID_EQUAL_BUTTON: 
      ...
      {
#if (defined ( __GNUC__ ))
        hEquWin = WM_CreateWindowAsChild(0, 0, 640, 480, pMsg->hWin, WM_CF_SHOW, _cbEqualDialog_des , 0);
        Hint = WM_CreateWindowAsChild(120,
                                      106,
                                      240, 60,
                                      pMsg->hWin,
                                      WM_CF_SHOW , 
    ...
    case ID_WAV_BUTTON: 
      ...
      case WM_NOTIFICATION_RELEASED:
        ...
        hItem = CHOOSEFILE_Create(pMsg->hWin,  95, 115, 450, 250, apDrives, GUI_COUNTOF(apDrives), 0, "Add a folder", 0, &FileInfo);        
        r = GUI_ExecCreatedDialog(hItem);
        if (r == 0) 
        {
    ...
    case ID_ADD_BUTTON: 
      ...
      case WM_NOTIFICATION_RELEASED:
        FileInfo.pfGetData = k_GetData;
        FileInfo.pMask = acMask_audio;     
        hItem = CHOOSEFILE_Create(pMsg->hWin,  95, 115, 450, 250, apDrives, GUI_COUNTOF(apDrives), 0, "Add to playlist", 0, &FileInfo);
        r = GUI_ExecCreatedDialog(hItem);
        if (r == 0) 
        {
          if((strstr(FileInfo.pRoot, ".wav")) || (strstr(FileInfo.pRoot, ".WAV")))
