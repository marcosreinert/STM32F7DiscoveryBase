...
USB device application based on the STM32F7x6 multi core support feature integrating the Device Communication 
Class (CDC) and Human Interface (HID) in the same project.

This is a typical application on how to use the STM32F7x6 USB OTG Device peripheral, where STM32 is 
enumerated as a CDC device in the High Speed mode, and also as a HID device in the Full Speed mode,
using the native PC Host HID/CDC drivers to which the STM327x6G-EVAL board is connected.
...
the STM327x6G_EVAL board, allow to user to emulate the Mouse directions.
...
@par Hardware and Software environment
  ...
  - STM327x6G-EVAL RevB Set-up
    - Connect the STM327x6G-EVAL board to the PC through two 'USB micro A-Male 
      to A-Male' cables to the connectors:
      - CN8 : to use USB High Speed (HS) 
      - CN13: to use USB Full Speed (FS)
    - Connect the STM327x6G-EVAL board to the PC (or to another evaluation board) through RS232 (USART)
      serial cable CN7 (USART1) connector.
      @note Make sure that jumper J7 is on RS232_RX position.
