...
__ALIGN_BEGIN uint8_t USBD_MSC_DeviceDesc[USB_LEN_DEV_DESC] __ALIGN_END = {
  ...
  LOBYTE(USBD_MSC_PID),           /* idVendor */
  HIBYTE(USBD_MSC_PID),           /* idVendor */
...
uint8_t *USBD_MSC_ProductStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  if(speed == 0)
  {   
    USBD_GetString((uint8_t *)USBD_MSC_PRODUCT_HS_STRING, USBD_MSC_StrDesc, length);
  }
  else
  {
    USBD_GetString((uint8_t *)USBD_MSC_PRODUCT_FS_STRING, USBD_MSC_StrDesc, length);    
  }
...
uint8_t *USBD_MSC_ConfigStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  ...
  {  
    USBD_GetString((uint8_t *)USBD_MSC_CONFIGURATION_HS_STRING, USBD_MSC_StrDesc, length);
  }
  else
  {
    USBD_GetString((uint8_t *)USBD_MSC_CONFIGURATION_FS_STRING, USBD_MSC_StrDesc, length); 
  }
  return USBD_MSC_StrDesc;  
}
...
uint8_t *USBD_MSC_InterfaceStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  if(speed == 0)
  {
    USBD_GetString((uint8_t *)USBD_MSC_INTERFACE_HS_STRING, USBD_MSC_StrDesc, length);
  }
  else
  {
    USBD_GetString((uint8_t *)USBD_MSC_INTERFACE_FS_STRING, USBD_MSC_StrDesc, length);
  }
  return USBD_MSC_StrDesc;  
}
