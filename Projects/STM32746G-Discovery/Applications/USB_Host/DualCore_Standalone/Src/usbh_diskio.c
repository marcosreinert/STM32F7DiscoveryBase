/**
  ******************************************************************************
  * @file    USB_Host/DualCore_Standalone/Src/usbh_diskio.c
...
DSTATUS disk_status(BYTE pdrv)
{
  ...
  if(USBH_MSC_UnitIsReady (&hUSBHost_HS, pdrv))
  {
    res = RES_OK;
  }
  else
...
DRESULT disk_read (BYTE pdrv, BYTE *buff, DWORD sector, UINT count)
{
  ...
  if ((DWORD)buff & 3) /* DMA Alignment issue, do single up to aligned buffer */
  {
    ...
    {
      status = USBH_MSC_Read(&hUSBHost_HS, pdrv, sector + count, (uint8_t *)scratch, 1);
      if(status == USBH_OK)
      {
  ...
  {
    status = USBH_MSC_Read(&hUSBHost_HS, pdrv, sector, buff, count);
  }
...
DRESULT disk_ioctl(BYTE pdrv, BYTE cmd, void *buff)
{
  ...
  case GET_SECTOR_COUNT:	
    if(USBH_MSC_GetLUNInfo(&hUSBHost_HS, pdrv, &info) == USBH_OK)
    {
